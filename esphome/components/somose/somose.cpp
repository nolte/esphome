...
static const uint8_t SOMOSE_MEASURE_CMD[] = {0x74,0x76};//{0x76,0x74};
...
void SoMoSeComponent::read_data_() {
  ...
  //uint32_t raw_moisture = ((uint32_t(data[2])) << 16) | (uint32_t(data[3]));
  
  ESP_LOGI(TAG,"%d",uint32_t(data[0]));
  // ESP_LOGI(TAG,"%d",uint32_t(data[1]));
  // ESP_LOGI(TAG,"%d",uint32_t(data[2]));
  // ESP_LOGI(TAG,"%d",uint32_t(data[3]));
  // ESP_LOGI(TAG,"%d",uint32_t(data[4]));
  // ESP_LOGI(TAG,"%d",uint32_t(data[5]));
